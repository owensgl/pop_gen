...
foreach my $chr (@uniq_chr){ #For each chromosome
	...
	foreach my $pos (sort {$a<=>$b} keys %{$poslist{$chr}}){
		if (($pos >= $s)and($pos<$e)){
			...
			if ($total{'variablecount'}){ #If there are variable sites in this window.
				...
				print "$chr\t$s\t$realend\t".($s+($Size/2))."\t$total{'usablecount'}\t$total{'variablecount'}\t$ave{'Hexp'}\n";
			}else{ #If there are usable sites but no variable sites
				unless ($total{'usablecount'}){
					$total{'usablecount'} = "0";
				}
				print "$chr\t$s\t$realend\t".($s+($Size/2))."\t$total{'usablecount'}\t0\t0\n";
			}
			# reset these
			$count = 1;
 			foreach my $stat (@statlist){
			...
			if ($h{'usablecount'}{$chr}{$pos}){
				...
				$total{'usablecount'} = 0;
                        ...
                        until ($pos<$e){
                                ...
                                $e += $Size;
				if ($pos>$e){
					print "$chr\t$s\t$realend\t".($s+($Size/2))."\t0\t0\tNA\n";
				}
                        }    
		}
	}
	if ($total{'variablecount'}){
		...
		print "$chr\t$s\t$realend\t".($s+($Size/2))."\t$total{'usablecount'}\t0\t0\t0\n";
	}
}
