...
while (<STDIN>){
	...
	if ($. == 1){
		...
		foreach my $i (0..$#a){
			...
			}elsif ($colname[$i] eq "Hexp_1"){
				$stat_hash{'Hexp_1'} = $a[$i];
			}elsif ($colname[$i] eq "Hexp_2"){
                                $stat_hash{'Hexp_2'} = $a[$i];
			}elsif ($colname[$i] eq "N1"){
				$stat_hash{'N1'} = $a[$i];
			}elsif ($colname[$i] eq "N2"){
                                $stat_hash{'N2'} = $a[$i];
			}
		}	#Take each stat and put it into a hash
		next if (($stat_hash{'N1'} eq "0") and ($stat_hash{'N2'} eq "0")); 
